    ...
    for doc1 in document_weight_sum.index:
        for doc2 in document_vectors:
            if doc1 == doc2:
                ...
                if ans != 0: # not adding results that have zero similarity with query and also only adding the first five results
                    if len(doc1) > 60: # alter name if it's too long for our GUI
